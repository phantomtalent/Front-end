...
@Component({
  selector: 'app-top-merchant',
  templateUrl: './top-merchant.component.html',
  styleUrls: ['./top-merchant.component.scss']
})
export class TopMerchantComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Top Merchants' }]);
  ...
  public search() {
    if (this.formGroup.valid) {
      ...
      this.merchantService.getTopMerchants({
        ...
        this.lineChart(
          'Merchant volumes IDS: ' + this.commaNumber(total),
          labels,
          dataSets,
          original
        );
  ...
  lineChart(label = 'Merchant volumes IDs', labels = [], dataSets = {}, original = []) {
