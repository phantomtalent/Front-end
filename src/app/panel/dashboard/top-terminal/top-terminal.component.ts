...
@Component({
  selector: 'app-top-terminal',
  templateUrl: './top-terminal.component.html',
  styleUrls: ['./top-terminal.component.scss']
})
export class TopTerminalComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Top Terminals' }]);
  ...
  public search() {
    if (this.formGroup.valid) {
      ...
      this.terminalService.getTopTerminals({
        ...
        this.lineChart(
          'Terminal volumes IDS: ' + this.commaNumber(total),
          labels,
          dataSets,
          original
        );
  ...
  lineChart(label = 'Terminal volumes IDs', labels = [], dataSets = {}, original = []) {
