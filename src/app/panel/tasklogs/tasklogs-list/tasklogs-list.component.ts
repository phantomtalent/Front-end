...
import { TaskLogsService } from '../../service/tasklogs.service';
import { BreadcrumbService } from '@app/panel/service/breadcrumb.service';
import { IPagination, ITaskLogs } from '@utils/interfaces';
...
export class TaskLogsListComponent implements OnInit {
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Task Logs' }]);
    const pagedResult: IPagedResult<ITaskLogs> = (<IPagedResult<ITaskLogs>>this.route.snapshot.data['pagedResult']);

    if (pagedResult) {
      this.tasklogs = pagedResult.content;
  ...
  search(resetPaging: boolean = true) {
    if (this.formGroup.valid) {
      ...
      this.taskLogsService.search(this.pagination, this.formGroup.value)
        .subscribe(result => {
          this.formGroup.enable();
          this.tasklogs = result.content;
