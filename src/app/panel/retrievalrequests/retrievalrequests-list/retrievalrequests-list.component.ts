...
import { IPagination, IRetrievalRequest } from '@utils/interfaces';
...
@Component({
  selector: 'app-retrievalrequests-list',
  templateUrl: './retrievalrequests-list.component.html',
  styleUrls: ['./retrievalrequests-list.component.scss']
})
export class RetrievalRequestsListComponent implements OnInit {
  ...
  retrievalrequests: IRetrievalRequest[];
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Retrieval Requests' }]);
    const pagedResult: IPagedResult<IRetrievalRequest> = (<IPagedResult<IRetrievalRequest>>this.route.snapshot.data['pagedResult']);

    if (pagedResult) {
      this.retrievalrequests = pagedResult.content;
  ...
  search(resetPaging: boolean = true) {
    if (this.formGroup.valid) {
      ...
      this.retrievalrequestService.search(this.pagination, this.formGroup.value)
        .subscribe(result => {
          this.formGroup.enable();
          this.retrievalrequests = result.content;
  ...
  export() {
    this.retrievalrequestService.export().subscribe(data => {
