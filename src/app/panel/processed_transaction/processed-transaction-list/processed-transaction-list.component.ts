...
import { IPagination, IProcessedTransaction } from '@utils/interfaces';
...
@Component({
  selector: 'app-processed-transaction-list',
  templateUrl: './processed-transaction-list.component.html',
  styleUrls: ['./processed-transaction-list.component.scss']
})
export class ProcessedTransactionListComponent implements OnInit {
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Processed Transactions' }]);
    const pagedResult: IPagedResult<IProcessedTransaction> = (<IPagedResult<IProcessedTransaction>>this.route.snapshot.data['pagedResult']);

    if (pagedResult) {
      this.processedTransactions = pagedResult.content;
  ...
  search(resetPaging: boolean = true) {
    if (this.formGroup.valid) {
      ...
      this.processedTransactionService.search(this.pagination, this.formGroup.value)
        .subscribe(result => {
          this.formGroup.enable();
          this.processedTransactions = result.content;
  ...
  export() {
    this.processedTransactionService.export().subscribe(data => {
