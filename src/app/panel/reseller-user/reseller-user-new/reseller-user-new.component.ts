...
@Component({
  selector: 'app-reseller-user-new',
  templateUrl: './reseller-user-new.component.html',
  styleUrls: ['./reseller-user-new.component.scss']
})
export class ResellerUserNewComponent extends FormBaseComponent implements OnInit {
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Reseller Users', slug: '/panel/resellerusers' }, { name: 'New' }]);

    this.resellerService.getResellersList()
      .subscribe(value => {
        if (value != null) {
          this.resellers = value;
          this.resellers.forEach(reseller => this.resellerselectdata.push(new SelectSearch(reseller.id, reseller.name)));
        }
      });
  }
  ...
  submit() {
    if (this.formGroup.valid) {
      ...
      this.resellerUserService.save(this.resellerUser).subscribe(() => {
