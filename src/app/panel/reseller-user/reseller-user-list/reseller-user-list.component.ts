...
@Component({
  selector: 'app-reseller-user-list',
  templateUrl: './reseller-user-list.component.html',
  styleUrls: ['./reseller-user-list.component.scss']
})
export class ResellerUserListComponent implements OnInit {
  ...
  ngOnInit() {
    this.breadcrumbService.setItems([{ name: 'Reseller Users' }]);
    const pagedResult: IPagedResult<IResellerUser> = (<IPagedResult<IResellerUser>>this.route.snapshot.data['pagedResult']);
  ...
  search(resetPaging: boolean = true) {
    if (this.formGroup.valid) {
      ...
      let filter: IResellerUserFilter = {};
      ...
      this.resellerUserService.search(this.pagination, filter)
        .subscribe(result => {
  ...
  export() {
    this.resellerUserService.export().subscribe(data => {
