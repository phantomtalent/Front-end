...
import {IPagination, ITaskLogs, ITaskLogsFilter} from '@utils/interfaces';
...
export class TaskLogsService {
  ...
  search(pagination?: IPagination, filter?: ITaskLogsFilter): Observable<IPagedResult<ITaskLogs>> {
    ...
    return this.http.get<IPagedResult<ITaskLogs>>(environment.api.urls.tasklogs.base, {params: pagedFilter});
  }

  private buildFilterParams(filter?: ITaskLogsFilter): HttpParams {
  ...
  get(acquirerId: number): Observable<ITaskLogs> {
    return this.http.get<ITaskLogs>(environment.api.urls.tasklogs.get(acquirerId));
  }
}
